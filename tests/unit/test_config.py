...
class TestConfig:
    ...
    def test_corpora_contains_expected_types(self):
        ...
        for code in expected_codes:
            assert code in Config.RNC_CORPORA

    def test_get_token_success(self, mock_env_token):
        """Test get_token() returns token when set."""
        token = Config.get_rnc_token()
        assert token == "mock_token_12345"
        assert len(token) > 0

    def test_get_token_missing_raises_error(self, clear_env_token):
        """Test get_token() raises error when token missing."""
        with pytest.raises(RNCConfigError) as exc_info:
            Config.get_rnc_token()

        assert "RNC_API_TOKEN is not set" in str(exc_info.value)
        assert "https://ruscorpora.ru/accounts/profile/for-devs" in str(
            exc_info.value)

    def test_get_token_empty_string_raises_error(self, monkeypatch):
        """Test get_token() raises error for empty string token."""
